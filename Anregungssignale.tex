...
Die Sprung- und die Impulsfunktion gehören zur Funktionenklasse der Distributionen. Sie zeichnen sich dadurch aus, dass sie mengenwertige Eigenschaften besitzen -- beispielsweise die elementweise Definition. 
...
\begin{itemize}
	...
	\item Für jede Funktion $f(x)$ gilt die Ausblendeigenschaft 
	\begin{align}
		\int_{-\infty}^{\infty}f(t)\delta(t-t_0) \textrm{d}t = f(t_0)		
	\end{align}
\end{itemize}
\subsection{Stoßantwort}
Die Anregung eines Einmassenschwingers durch einen Stoß (z.B. Hammerschlag) kann formal über den Dirac-Impuls eingeführt werden. Zunächst lautet die DGL 
\begin{align}
	m\ddot x + d\dot x + c x = \delta(t)
	\label{eq:EMSImpuls}
\end{align}
wobei für den Dirac-Impuls $\delta(t)$ die abschnittsweise Definition in Abhängigkeit von $\varepsilon$ verwendet wird. Dementsprechend ist die Lösung eine abschnittsweise definierte Funktion: 
\paragraph{Vor Stoß ($t \leq -\varepsilon $)}
Die Masse ist in Ruhe. Also sind Lage und Geschwindigkeit 
	\begin{align}
	&x(-\varepsilon)=0&  &\dot x(-\varepsilon)=0&
	\end{align}
\paragraph{Während des Stoßes ($-\varepsilon  \leq t \leq \varepsilon$)}
Die Bewegungsgleichung lautet 
	\begin{align}
	m\ddot x + d\dot x + c x = \frac{1}{2\varepsilon}
	\end{align}
	Die Integration über die Stoßdauer ergibt in mehreren Schritten  
	\begin{align}
	...
	+c \int_{-\varepsilon}^{\varepsilon}x\textrm{d}t = 1
	\end{align}
	Unter der Annahme, dass die Lage $x(t)$ während der Stoßdauer stetig bleibt, folgt $x(-\varepsilon)= x(\varepsilon)=x_0$ und damit 
	\begin{align}
	...
	\end{align}
\paragraph{Nach Stoß ($\varepsilon  \leq t $)} 
Die Bewegungsgleichung lautet mit Anfangsbedingungen
	...
	\begin{align}
		x(t) &= \frac{1}{m\omega_d}e^{-D\omega_0 t}\sin(\omega_d t)
	\end{align}
Die Impulsantwort (Antwort auf einen mechanischen Stoß) wird gerne als Testsignal verwendet. Es lässt sich über einen Hammerschlag realisieren: Das System wird kurz angestoßen und dann sich selbst überlassen. Aus der Stoßantwort lassen sich die Systemparameter $m$, $d$, $D$ bestimmen. 
...
Eine harmonische Schwingung lässt sich im Frequenzbereich vollständig durch Amplitude $\hat x$, Frequenz $\omega_0$ und Nullphasenwinkel $\varphi$ beschreiben. Diese Eigenschaft motiviert die Darstellung im Zeit- und im Frequenzbereich:
\begin{figure}[H]
...
\caption{Reines Cosinussignal $x(t) = \hat{x}\cos (\omega t) = Re(\hat{x} e^{\omega t})$}
\end{figure}
\begin{figure}[H]
	\subfloat[Zeitbereich]{
		\begin{tikzpicture}
	...
	\caption{Reines Sinussignal $x(t) = \hat{x}\sin (\omega t)  = Re(\hat{x} e^{\omega t-\pi/2})$}
\end{figure}
\begin{figure}[H]
\subfloat[Zeitbereich]{
	\begin{tikzpicture}
...
\caption{Allgemeines Signal $x(t) = \hat{x}\cos (\omega t+\varphi) = Re(\hat{x} e^{\omega t+\varphi})$}
\end{figure}
...
Es sei eine $T$-periodische Funktion mit der Grundfrequenz $\omega_0 = \frac{2\pi}{T}$ gegeben. Unter den Voraussetzungen 
\begin{itemize}
	\item endlich viele Sprungstellen endlicher Höhe
	\item 2-Fache Integrierbarkeit von $x(t)$
\end{itemize}
lässt sich die Funktion $x(t)$ als \emph{Fourierreihe} darstellen:
\begin{theo}[Fourierreihe]{theo:Fourierreihe}
Die Entwicklung von $x(t)$ als Fourierreihe lautet 
\begin{align}
	x(t) = C_0 + \sum_{k=1}^{\infty}C_k \cos (k\omega_0 t) + S_k \sin (k\omega_0 t) \nonumber 
\end{align}
wobei die Koeffizienten des Polynoms nach 
\begin{align}
	C_0 & = \frac{1}{T}\int_{-T/2}^{T/2}x(t) \textrm{d}t \nonumber\\
	C_k & = \frac{2}{T}\int_{-T/2}^{T/2}x(t)\cos(k\omega_0 t)\textrm{d}t& 
	&k\in \mathbb{N}&\nonumber\\
	S_k & = \frac{2}{T}\int_{-T/2}^{T/2}x(t)\sin(k\omega_0 t)\textrm{d}t& 
	&k\in \mathbb{N}&\nonumber
\end{align}
berechnet werden. 
\end{theo}
...
\begin{align}
x(t) = C_0 + \sum_{k=1}^{\infty}\hat x_k \cos (k\omega_0 t+\varphi_k) \nonumber 
\end{align}
...
\end{theo}
\begin{theo}[Darstellung der Fourierreihe also komplexes Polynom]{theo:Fourierreihekomplex}
		\begin{align}
		x(t) = \sum_{k=-\infty}^{\infty} a_k e^{i k\omega_0 t} \nonumber 
		\end{align}
		wobei die Koeffizienten des Polynoms nach 
		\begin{align}
		a_k & = \frac{1}{T}\int_{0}^{T}x(t)e^{-i k\omega_0 t}\textrm{d}t & 
...
\begin{itemize}
	...
	\item Der Anteil $\hat x_1\cos (\omega_0 t+\varphi_1)$ heißt \emph{Grundschwingung} oder \emph{1. Harmonische}
	\item Der Anteil $\hat x_2\cos (2\omega_0 t+\varphi_2)$ heißt \emph{1. Oberschwingung} oder \emph{2. Harmonische}
	\item Der Anteil $\hat x_k\cos (k \omega_0 t+\varphi_k)$ heißt \emph{(k-1)-te Oberschwingung} oder \emph{k-te Harmonische}
...
\begin{align}
	&S_k = 0&
	&C_k = \frac{4}{T}\int_{0}^{T/2} x(t) \cos (k\omega_0 t) \textrm{d}t&
\end{align}
\paragraph{Ungerade Funktionen.} Für ungerade Funktionen $x(t)$ gilt $x(t)=-x(-t)$. Dann folgt: 
\begin{align}
&S_k = \frac{4}{T}\int_{0}^{T/2} x(t) \sin (k\omega_0 t) \textrm{d}t&
&C_k = 0&
\end{align}
...
\begin{align}
	...
	\right. 
\end{align}
Die Grundfrequenz ist $\omega_0=2\pi/T$. Diese Funktion ist punktsymmetrisch zum Ursprung, also ungerade. Daher ist
\begin{align}
	C_k&=0, k\in \mathbb N_0,\\
	S_k&=\frac{4}{T}\int_{0}^{T/2}x(t)\sin(k\omega_0 t)\textrm{d}t\\
	& =-\frac{4}{T k \omega_0}\left(\cos\left(k\omega_0 \frac{T}{2}\right)-1\right)\\
	...
	\right. 	
\end{align}
Die Reihenentwicklung der Rechteckfunktion $x(t)=\sum_{k} S_k \sin (k\omega_0 t)$ hat wegen $x(t)= \sum_{k} S_k \cos (k\omega_0 t-\pi/2)$ die Amplituden $\hat x_k=S_k$ und die Phasen $\varphi_k =-\pi/2$. Die Darstellung im Zeit- und im Frequenzbereich ist wie folgt: 
\begin{figure}[H]
	\subfloat[Zeitbereich]{
		\begin{tikzpicture}
		\begin{axis}[
...
C_k& =\frac{2}{T}\int_{0}^{T}x(t)\cos (k \omega_0 t)\textrm{d}t =\frac{2}{T}\left(\int_{-\tau/2}^{\tau/2}\frac{1}{\tau}\cos (k \omega_0 t)\textrm{d}t +\int_{\tau/2}^{T-\tau/2}0\textrm{d}t\right)\\
& =2\cdot \frac{2}{T k \omega_0}\frac{1}{\tau}\left(\sin\left(k\omega_0 \frac{\tau}{2}\right)\right)\\
&= \frac{2}{k\pi \tau}\sin\left(k\pi\frac{\tau}{T}\right)
\end{align}
Somit lautet die Fourierreihe der Rechteckimpulsfolge
\begin{align}
	x(t) = \frac{1}{T} + \sum_{k=1}^{\infty}\frac{2}{k\pi \tau}\sin\left(k\pi \frac{\tau}{T}\right)\cos (k\omega_0 t)
\end{align}
...
\right. 
\end{align}
Auch diese Funktion ist symmetrisch, weshalb die Koeffizienten $C_k$ allesamt verschwinden. Ferner ist 
\begin{align}
S_k&=\frac{4}{T}\int_{0}^{T/2}x(t)\sin(k\omega_0 t)\textrm{d}t\\
& = \left\{
\begin{array}{ll}
...
\label{eq:PolyH}
\end{align}
Diese Lösung hat Eigenschaften wie die Schwingungsantwort auf eine Einzelanregung. Die Resonanz des ungedämpften Einmassenschwingers wird jetzt nicht mehr nur bei $\Omega=\omega_0$, sondern bei $k \Omega=\omega_0$ mit allen $k\in \mathbb N$ angeregt. 
\begin{beispiel}[Motorblockanregung]{beispiel:Motorblockanregung}
Die Vertikalbewegung eines Motorblocks kann durch die bekannte DGL des 1-Massen-Schwingers beschrieben werden. Die Schwingungsanregung durch Zündung der 4 Zylinder hat ihre Hauptkomponente bei doppelter Motordrehzahl, eine weitere wichtige Komponente ist die 4. Dehzahlordnung. Wenn die Anregung vereinfachend konstante Amplituden hat, dann lautet die Bewegungsgleichung 
\begin{align}
...
\end{align}
Diese Aufgabe kann numerisch gelöst werden. In \emph{Octave} lautet eine mögliche Implementierung 
...
Für periodische Funktionen gilt: $\omega_0 = \frac{2\pi}{T}$. Die Grundfrequenz ist also durch die endliche Periodenlänge begrenzt. Für nichtperiodische Funktionen (z.B. einmalige Vorgänge) geht die Schwingungsdauer $T$ gegen unendlich. Damit ist auch die Grundfrequenz $\omega_0\rightarrow \textrm{d}\omega$ (sie wird differenziell klein). 
Mit Darstellung von (Mathematik 4.5) lauten die komplexen Schwingungskomponenten im Grenzübergang
\begin{align}
x(t) &= \sum_{k=-\infty}^{\infty}a_k e^{i k\omega_0 t} = \sum_{k=-\infty}^{\infty}\frac{1}{T}\underbrace{\int_{0}^{T}x(t)e^{-i k\omega_0 t}\textrm{d}t}_{=: X_k = X(k\omega_0)} e^{i k\omega_0 t}\\
...
\begin{align}
\mathcal{F}(\sin(\omega_0 t)) &= i \pi \left(\delta(\omega+\omega_0)-\delta(\omega-\omega_0)\right)\\
...
\end{align}
Bemerkenswert an diesem Ergebnis sind zwei Beobachtungen: Das bisherige diskrete Frequenzspektrum (Grundfrequenz $\omega_0$) geht nun über in ein verteiltes Spektrum, für das alle reellen Werte von $\omega$ zugelassen sind; die weiteren Eigenschaften des bisherigen Ergebnisses sind beibehalten. Ferner folgt für den Grenzübergang $\tau\rightarrow0$ das Ergebnis von Beispiel 4.5. 
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	\begin{axis}[
...
Die Fouriertransformation angewandt auf die Gleichung (4.3) lautet gemäß der Rechenregeln: 
\begin{align}
